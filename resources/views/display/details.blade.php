            ...
            <div class="container mx-auto px-6">
                <div class="md:flex md:items-center">
                    <div class="w-full h-64 md:w-1/2 lg:h-96">
                        <img class="h-full w-full rounded-md object-cover max-w-lg mx-auto" src="{{ asset('storage') . '/' . $products->images[0]->name }}" alt="Product Image">
                    </div>
...
{{--                            </button>--}}
    ...
    <script>
        ...
        // Price value
        const discounts = @json($products->discounts);
        const inventary = @json($products->inventary);
        ...
        function addToCart(pivotID, quantity,productId) {
            ...
            cart.push({ id, quantity, user_id,productId });
        ...
        const product = @json($products->toArray());
        ...
        document.getElementById('addToCartButton').addEventListener('click', function() {
            ...
            const sizes = @json($products->sizes);
            var size = sizes[selectedSizeIndex].size;

            var id = product.id;

            $.ajax({
                url: "{{ route('products.checkInventary', $products->id) }}",
                type: 'POST',
                headers: {
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        addToCart(response.pivot_id,quantity,id);
